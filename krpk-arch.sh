...
#part1
printf '\033c'
echo "welcome to krpk's arch installer script"
sed -i "s/^#ParallelDownloads = 5$/ParallelDownloads = 15/" /etc/pacman.conf
pacman --noconfirm -Sy archlinux-keyring
loadkeys us
timedatectl set-ntp true
lsblk
echo "Note: If you create a swap partition then create it as a first partition(/dev/sda1) of your disk)...."
echo "Note: Create a linux partition as /dev/sda2 (Second partition as linux)...."
...
read partition
mkfs.ext4 $partition 
read -p "Did you also created swap partition? [y/n]" answer
if [[ $answer = y ]] ; then
  mkswap /dev/sda1
  swapon /dev/sda1
fi
...
printf '\033c'
cd $HOME
git clone --separate-git-dir=$HOME/.dotfiles https://github.com/kr-pk/dotfiles.git tmpdotfiles
tar -xf $HOME/tmpdotfiles/dotfiles.tar.gz
rsync --recursive --verbose --exclude '.git' tmpdotfiles/dotfiles $HOME/
rm -rf tmpdotfiles
...
#If you want to install zsh then uncomment these lines....
